        ...
        do {
            ...
            switch (choice) {
                case "1":
                    System.out.println("Enter the name of the food:");
                    foodName = sc.nextLine();
                    System.out.println("Enter the price of the food (double):");
                    foodPrice = sc.nextDouble();
                    sc.nextLine(); // nextDouble() takes the double but does not consume the \n comes after the value, we must consume it
                    System.out.println("Enter [n] if this food does not contain meat, otherwise it will be assumed to have meat:");
                    hasMeat = sc.nextLine();
                    System.out.println("Enter the ingredients of the food (in a single line using comma (,) to separate items):");
                    ingredients = sc.nextLine();
                    containsMeat = hasMeat.equals("n") ? false : true;
                    ItemController.addAFoodToShop(seller, foodName, foodPrice, containsMeat, new ArrayList<>(Arrays.asList(ingredients.split(","))));
                    break;
                case "2":
                    System.out.println("Enter the name of the drink:");
                    drinkName = sc.nextLine();
